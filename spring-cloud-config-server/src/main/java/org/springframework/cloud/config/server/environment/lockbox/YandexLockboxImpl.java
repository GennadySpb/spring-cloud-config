...
package org.springframework.cloud.config.server.environment.lockbox;
...
public class YandexLockboxImpl implements YandexLockbox {
	...
	private static Log logger = LogFactory.getLog(org.springframework.cloud.config.server.environment.lockbox.YandexLockboxImpl.class);
	...
	public List<Secret> getSecrets() {
		...
		ProjectName project = ProjectName.of(getFolderId());
	...
	@Override
	public String getSecretValue(SecretOuterClass.Secret secret, Comparator<SecretOuterClass.Version> comparator) {

	}

	@Override
	public String getSecretName(SecretOuterClass.Secret secret) {
		return null;
	}
	...
	public Boolean checkRemotePermissions() {
		...
		try {
			...
			TestIamPermissionsResponse testIamPermissionsResponse = service.projects()
				.testIamPermissions(getFolderId(), requestBody).execute();
